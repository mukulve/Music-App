...
export function Raggae({ navigation }) {
  const options = {
    method: "GET",
    headers: {
  ...
  let raggae = ["2448918882", "2042023484", "1503415633", "1973876342"];
  ...
  useEffect(() => {
    const fetchPlaylists = async () => {
      for (let i = 0; i < raggae.length; i++) {
        const response = await fetch(
          "https://deezerdevs-deezer.p.rapidapi.com/playlist/" + raggae[i],
          options
        );
